...
class Initializer():
    """
    To build pathology verification model, one needs speech data from each pathology
    that is to be known by the system. The set of known pathologies are in pathology
    recognition known as the (enrollment pathologies), and a pathology is enrolled
    into the system when enrollment data from the pathology is processed to build
    its model.
    After the enrollment process, the performance of the pathology verification
    system can be evaluated using test data, which in an open set scenario, will
    consist of data from pathologies in and outside the enrollment set.
    The set of all pathologies involved in testing the system will be referred to
    as the test pathologies.
    ...
    def preprocess_audio(self):
        ...
        pathology = sorted(os.listdir(self.conf['inpath']))
        for sp in tqdm(pathology, desc="Converting Audio"):
            pathology_path = os.path.join(self.conf['inpath'], sp)
            wav_filenames = os.listdir(pathology_path)
            for wav in wav_filenames:
                inwav = os.path.join(pathology_path, wav)
    ...
    def create_idMap(self, group):
        ...
        stored in leftids; with the ID of their class (that could be the pathology
    ...
    def create_test_trials(self):
        ...
        test_segments = []
        test_labels = []
        # Get enroll pathologies
        enrolled_pathologies = set([])
        for filename in os.listdir(os.path.join(self.audio_dir, "enroll")):
            enrolled_pathologies.add(filename.split(".")[0])
        enrolled_pathologies = sorted(enrolled_pathologies)
        for model in tqdm(enrolled_pathologies, desc="Creating Test Cases"):
